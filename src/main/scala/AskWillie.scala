...
@main def main(): Unit = {
        ...
        while {
          ...
          val searchedPages: List[SearchedWebPage] = rankedPages.zip(PageSearch.tf(rankedPages, terms)).map((page, textmatch) => SearchedWebPage(page, textmatch)) // call PageSearch.???? here
